...
EveChats keeps application identity, Agent authorization, and local browser
...
- When Eveland explicitly reports an open-access configuration, the gate steps
  aside and the signed EveChats browser session owns anonymous history.
...
| EveChats `eve_chats_session` cookie | Anonymous browser ownership and legacy-chat claiming | No |
| Eveland App Token | Identity-scoped EveChats history and mutations | No |
...
expiry. EveChats verifies their signatures against Eveland JWKS. A Caller Token
...
The signed, HttpOnly EveChats browser cookie remains in use even on authenticated
...
When Eveland Identity and EveChats use the same hostname on different ports,
...
Production should place Eveland Identity and EveChats on one HTTPS schemeful
...
| `NEXT_PUBLIC_EVELAND_IDENTITY_RETURN_TARGET` | Registered Eveland return target; defaults to `eve-chats` |
...
Register the exact EveChats origin as the return target in Eveland System >
